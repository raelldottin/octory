...
OCTO_CMD="/Library/Application Support/Octory/Resources/bin/octo-notifier"
...
set_datetime() {
    ...
    print_log "Syncing system time to NTP server ${NTP_SERVER}"
    execute "sntp" "-sS" "${NTP_SERVER}"
}
...
wait_for_octory_helper() {
    ...
    until [[ -n "${octory_helper}" ]]; do
        ...
        octory_helper=$(tail -4 /tmp/"${ACTIVE_USER}"-octory.log 2> /dev/null | grep "All applications not installed" 2> /dev/null)
    done
    sleep 3
}
