...
exec   > >(tee -ia /var/tmp/bash.log)
exec  2> >(tee -ia /var/tmp/bash.log >& 2)
exec 19> /var/tmp/bash.log
...
YOUR_CONNECT_KEY="(connect key placeholder)"
...
DEP_ENROLLMENT_GROUP_ID="60257f64b544065d1eaf1810"

### JumpCloud System Group ID For DEP POST Enrollment ###
DEP_POST_ENROLLMENT_GROUP_ID="602580371f2475728148a12e"

### DEPNotify Welcome Window Title ###
WELCOME_TITLE="Welcome to Foursquare!"

### DEPNotify Welcome Window Text use \n for line breaks ###
WELCOME_TEXT="Welcome to Foursquare! We can't wait for you to get started with your new MacBook! \n\nSit back and relax while we set a few important things up for you!"
...
DEP_N_OCTO="/Library/Application Support/Octory/Octory.app/Contents/MacOS/Octory"
DEP_N_OCTOCMD="/usr/local/bin/octo-notifier"
...
if [[ ! -f $DEP_N_GATE_INSTALLJC ]]; then
    ...
    "$DEP_N_OCTO" --config fsq_pre.plist &>/dev/null &

    sleep 10

    "$DEP_N_OCTOCMD" monitor JCInstall -s installing
    sleep 3

    "$DEP_N_OCTOCMD" monitor Connect -s installing
    sleep 3

    "$DEP_N_OCTOCMD" monitor Config -s installing
    sleep 3
...
EOF
    ...
    "$DEP_N_OCTOCMD" monitor JCInstall -s installed
...
if [[ ! -f $DEP_N_GATE_SYSADD ]]; then
    ...
    "$DEP_N_OCTOCMD" monitor Connect -s installed
    ...
    "$DEP_N_OCTOCMD" monitor Config -s installed
...
if [[ ! -f $DEP_N_GATE_UI ]]; then
    ...
    if [[ $SELF_ID == "CE" ]]; then
        CompanyEmail=$(defaults read "/Library/Application Support/Octory/Resources/Inputs/inputs.plist" ceMail)
    elif [[ $SELF_ID == "PE" ]]; then
    ...
    while [ "$LOCATED_ACCOUNT" == "False" ]; do
        ...
        "$DEP_N_OCTOCMD" monitor Activate -s installing
        sleep 3

        "$DEP_N_OCTOCMD" monitor Create -s installing
        sleep 3

        "$DEP_N_OCTOCMD" monitor Finish -s installing
        sleep 3
        ...
        if [[ $search_step -ge 1 && "$totalCount" != "1" ]]; then
            ...
            "$DEP_N_OCTOCMD" trigger jumpError
    ...
    "$DEP_N_OCTOCMD" monitor Activate -s installed
    touch /var/tmp/com.depnotify.provisioning.done
...
if [[ ! -f $DEP_N_GATE_DONE ]]; then
    ...
    "$DEP_N_OCTOCMD" monitor Create -s installed
    ...
    "$DEP_N_OCTOCMD" monitor Finish -s installed

    sleep 3

    "$DEP_N_OCTOCMD" trigger jumpLast
    ...
    FINISH_TEXT='This window will automatically close when configuation is complete.\n\nYou will be taken directly to the log in screen. \n\nUse your Foursquare JumpCloud information to log into your macOS account! \n\n'
